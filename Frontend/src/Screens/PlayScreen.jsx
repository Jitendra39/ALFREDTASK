...
const PlayScreen = () => {
  ...
  useEffect(() => {
    const updateFlashcard = async () => {
      try {
        const response = await fetch(`${url}/flashcards/${id}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ level, win, matchCount, id }),
        });
  ...
  const getAllCards = async () => {
    try {
      const response = await fetch(`${url}/flashcards?id=${id}`);
      const data = await response.json();
      console.log("data", data);
      if (data.imageArray) {
        setApiImages(
          data.imageArray.map((item) => `${url}/Images?add=${item.url}`)
        );
  ...
  const imagesSource = apiImages.length > 0 ? apiImages : defaultFruits;
  ...
  return (
    <div className="relative flex flex-col justify-center items-center min-h-screen mt-14 md:mt-8 px-2">
      {/* Floating Level & Match Counter */}
      <div className="flex flex-col items-center pb-10">
        <motion.p
          className="special-text text-lg sm:text-xl md:text-2xl font-bold text-white"
          animate={{ x: [10, 0, 10] }}
      ...
      <div className="flex flex-col gap-2 p-2 sm:p-4 bg-transparent rounded-lg shadow-lg w-full max-w-lg md:hidden">
      ...
      <div className="hidden md:flex flex-col gap-2 p-2 sm:p-4 bg-transparent rounded-lg shadow-lg w-full max-w-lg">
        {displayDeck.length > 0 &&
      ...
      {!win && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-75">
          ...
          <Button text="Play Next Level" onClick={() => window.location.reload()} />
        </div>
      )}
    </div>
  );
